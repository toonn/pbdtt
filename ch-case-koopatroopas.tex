...
types met argumenten met kind \ihask{*} en \ihask{Fin} heeft een argument met
kind \ihask{Nat}.
...
\ihask{Path} kan geen parameter hebben met kind \ihask{KoopaTroopa} omdat het
type \ihask{KoopaTroopa} een parameter heeft met een kind, namelijk
...
kunnen die niet \emph{verbruiken} en het type \ihask{Path} heeft ook de
...
\ihask{Matrix} heeft parameters met kind \ihask{Nat} en kan dus niet
gepromoveerd worden. Dit kunnen we oplossen door natuurlijke getallen te
definiëren met kind \ihask{*}: een leeg type \ihask{Z} en een leeg type
...
omdat \ihask{Vec} weer een parameter van kind \ihask{Nat} heeft. Dit kunnen we
oplossen door het kind \ihask{Nat} in de definitie van \ihask{Vec} te vervangen
...
signature voor \ihask{redNoPathOne} geven. We moeten dus goed oppassen met
...
overeen met het type dat door \ihask{Pcons} word afgedwongen. Als we de type
